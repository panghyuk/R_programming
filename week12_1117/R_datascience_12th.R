...
setwd('C:/Users/USER/Documents/data/r_datascience_12th_data')
...
pred = predict(rf)
rf_df = cbind(loan3000 , pred_default = pred[ , 'default']>0.5)
...
loan3000 <- select(loan3000, -X)
rf_all <- randomForest(outcome ~ ., data=loan3000, importance=TRUE)
rf_all
...
sp = read.csv('Student Performance new.csv')
